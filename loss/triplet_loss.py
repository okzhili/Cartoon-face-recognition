...
def hard_example_mining(dist_mat, labels,cps,return_inds=False):
    ...
    is_neg = (labels.expand(N, N).ne(labels.expand(N, N).t()))*(cps.expand(N, N).ne(cps.expand(N, N).t()))

    # `dist_ap` means distance(anchor, positive)
    # both `dist_ap` and `relative_p_inds` with shape [N, 1]
    dist_ap, relative_p_inds = torch.max(
        dist_mat[is_pos].contiguous().view(N, -1), 1, keepdim=True)
    #dist_mat print(dist_mat[is_pos].shape)
...
class TripletLoss2(object):
    ...
    def __call__(self, global_feat, labels, normalize_feature=False):
        ...
        dist_ap, dist_an = hard_example_mining(dist_mat, labels,cps)
